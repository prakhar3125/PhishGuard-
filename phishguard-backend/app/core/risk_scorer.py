...
class RiskScorer:
    def __init__(self):
        # We don't use weights anymore; we use the "Max" strategy 
        # so one high indicator triggers an alert.
        pass
    ...
    def calculate_risk_score(self, analysis_data: Dict) -> Dict:
        ...
        if threat_intel_results:
            if any(r.get('is_malicious') for r in threat_intel_results):
                ...
                if threat_intel_results:
                    avg_reputation /= len(threat_intel_results)
                scores['threat_intel_score'] = min(int(avg_reputation), 100)
        ...
        # --- CRITICAL FIX: MAX STRATEGY ---
        # Don't average. If one part screams "DANGER", the whole email is dangerous.
        ...
        # --- CRITICAL FIX: RETURN FORMAT ---
        # The frontend expects 'breakdown' with these specific keys.
        return {
    ...
    def _calculate_heuristic_score(self, analysis_data: Dict) -> int:
        ...
        matches = [word for word in red_flag_words if word in full_text]
        score += len(matches) * 10

        # Critical Keywords (CEO Fraud / Wire Transfer) - Adds huge score
        critical_keywords = ['wire transfer', 'bitcoin', 'gift card']
